...
# - Alumno 1: Elena María Gómez Orihuel
# - Alumno 2: Dante Adami Peracchia
# - Alumno 3: Javier Montoya Gutiérrez de Ravé
...
# 
# Follow the link below to download the classification data set  â??xview_recognitionâ?: [https://drive.upm.es/s/UeDNT7JEYFkCVPP](https://drive.upm.es/s/UeDNT7JEYFkCVPP)
...
# Define data augmentation parameters for helicopter images
...
# #### Training
# Design and train a ffNN to deal with the â??xview_recognitionâ? classification task.
...
#model.add(Conv2D(128, (3, 3), activation='gelu', kernel_initializer="he_normal"))
#model.add(BatchNormalization())
#model.add(Dropout(0.25))
#model.add(MaxPooling2D(pool_size=(2, 2)))
...
model.add(Dense(100, activation = 'gelu', kernel_initializer = 'he_normal', kernel_regularizer=keras.regularizers.l1_l2(0.0001)))
model.add(BatchNormalization())
...
"""
optimizer = Adagrad(lr=1e-3, epsilon=1e-08, weight_decay=0.0001)
...
opt = RMSprop(lr=0.001, momentum=0.9, decay=0.0001)
model.compile(optimizer=opt, loss='categorical_crossentropy', metrics=['accuracy'])
"""
"""
...
for idx in range(len(categories)):
    ...
    # Dice = 2 |Aâ?ŠB| / (|A|+|B|) = 2 TP / (2 TP + FP + FN)
